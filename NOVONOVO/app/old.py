...
def upload_recorded():
    audio_blob = request.data  # Read the audio blob from the request body

    # Upload the audio blob to Google Cloud Storage
    blob = bucket.blob('uploaded_audio-duarte.wav')  # Specify a filename for the audio
    blob.upload_from_string(audio_blob, content_type='audio/wav')

    # Transcribe the uploaded audio
	
    #client = speech.SpeechClient()
    #audio = speech.RecognitionAudio(uri=f'gs://{bucket_name}/uploaded_audio.wav')
    #config = speech.RecognitionConfig(
    #    encoding=speech.RecognitionConfig.AudioEncoding.LINEAR16,
    #    sample_rate_hertz=16000,
    #    language_code='en-US'
